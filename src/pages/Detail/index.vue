          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: attrlist.isChecked == 1 }"
                  v-for="attrlist in attr.spuSaleAttrValueList"
                  :key="attrlist.id"
                  @click="ChangeListClass(attrlist, attr.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="shoppcart"
                  @change="ShoppCartChange"
                />
                <a href="javascript:" class="plus" @click="shoppcart++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="shoppcart > 1 ? shoppcart-- : (shoppcart = 1)"
                  >-</a
                >
              </div>
              <div class="add">
                <a href="javascript:" @click="JoinShppingCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 购物车数量规则
    ShoppCartChange() {
      let value = this.shoppcart * 1;

      if (isNaN(value) || value < 1) this.shoppcart = 1;

      this.shoppcart = parseInt(this.shoppcart);
    },

    // 添加购物车
    async JoinShppingCart() {
      try {
        await this.$store.dispatch("sendshoppingCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.shoppcart,
        });

        // 跳转到购物车添加成功   传数量
        this.$router.push({name: 'AddCartSuccess',query:{skuNum: this.shoppcart}})

        // 将数据传给组件
        sessionStorage.setItem('setSkuInfoData',JSON.stringify(this.skuInfo))
      } catch (error) {
        alert(error.message);
      }
    },
